...
function manage_sec_files()
{
	...
	$(document).ready(function(){
		$(".sec-file-tab").click(function(){
			id = $(this).attr("id");
			table_name = id.replace("tab","table");
			//alert(table_name); 
			$(".tab-section").hide();
			$("#" + table_name).show();
		});
	...
	<a href="#" id="tab-sec-files" class=" sec-file-tab nav-tab  '.$css_tab_published.'">Docs</a>
	<a href="#" id="tab-xbrl-files" class=" sec-file-tab nav-tab '.$css_tab_published.'">XBRL Files</a>
	<a href="#" id="tab-instructions" class=" sec-file-tab nav-tab '.$css_tab_published.'">Instructions</a>
	</h2>
	');
	...
	echo "<h2>Manage SEC Files</h2>";
			...
			?>
			<link rel='stylesheet' type='text/css' href='http://www.tracintermodal.com/wp-content/plugins/ti_deployment/css/ti_deployment.css'/>
...
echo "</div>";
			...
			if(isset($_POST['y'])) {
				...
				// Generate the XBRL Report
					if($_POST['generate_report']=='on')
					{
						...
						echo "<h4 style='color:#F00;'>No report was generated </h4>";
					}
			...
			<strong>Report Details<?php echo $spaces;?>
			<input type="checkbox" id="generate-report" name="generate_report"> <label for="generate-report">Generate XBRL Report</label></strong><br/>
			<div id="generate-report-form" style='display:block;'>
			Report Type 
			<select name='t'>
			<option>8K</option>
			<option>10K</option>
...
function processUnZip($sourceFileName,$destinationPath){
  if (stristr(PHP_OS, 'WIN')) {
   $this->unZipOnWindows($sourceFileName,$destinationPath);
  } else {
   $this->unZipOnLinux($sourceFileName,$destinationPath);
  }
 }
