...
****************************************/
	...
	function jqmRemoveAllMarkers()
	{
		jqmMap.removeMarkers();
		jqmMap.refreshMarkers();
	}
	
	function jqmShowRandomMarkers(map_type)
	{
		jqmMap.removeMarkers();
		url = "jquerymaps/theme/marker_states.php?map_type=" + map_type;
		jqmMap.loadMarkers(url); 
	}
	...
	function jqmDisplayGoogle(id, lat, lon) {
		...
		$.ajax({url: "jquerymaps/data/location_google.php", data: "id=" + id, dataType: "json", cache: false, success: function(data) {
		    ...
		    $.each(list, function (index, item) {
				...
				google.maps.event.addListener(marker, 'mouseover', function() { map_popup.setContent(marker.title); map_popup.open(map, this); });
